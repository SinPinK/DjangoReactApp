## Простое SPA-приложение на Django-React
 ...
 - Во время создания будем сразу использовать систему контроля версии git, требуется аккаунт GitHub
...
##### `git remote add origin git@github.com:<НАИМЕНОВАНИЕ В ГИТХАБ>`
...
#### 0.2. Клонирование из чужого репозитория и запись в свой
#### 0.2.1. Клонирование:
##### `git clone <repo_name>`
 - `repo_name` - из поля `code` в репозитории проекта
#### 0.2.2. Удаление URL-адреса, указывающего на `origin`
##### `git remote remove origin`
#### 0.2.2. Проверка имеющихся URL-адресов 
##### `git remote -v`
 - после выполнения команды `git remote remove` результат `git remote -v` должен быть пустым
#### 0.2.3. Обновление URL-адреса 
#####  `git remote add origin <repo_name>`
 - добавление нового URL-адреса для remote
 - `repo_name` из репозитория в github, в который требуется запушить изменения
#### 0.2.4. Создание ветки для записи в репозиторий
 - без ветки вы не сможете запушить изменения в репозиторий github
##### `git branch <new_branch_name>`
#### 0.2.5. Загрузка кода в репозиторий github
##### `git add .`
